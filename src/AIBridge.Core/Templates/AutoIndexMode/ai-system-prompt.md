...
4. **Keeping the Index Current**:
   The index is a snapshot and will drift out of sync as code changes are applied. To prevent this, you MUST automatically include an `<update-index>` block inside your `<ai-response>` (as described in `ai-update-index-skill.md`) whenever your code changes affect a file's overall purpose. See `ai-response-skill.md` for the exact placement.
