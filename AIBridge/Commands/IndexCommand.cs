...
/// <summary>
/// Handles viewing and comparing the ai-bridge index.
/// </summary>
public class IndexCommand
{
    private readonly string _projectRoot;

    public IndexCommand(string projectRoot)
    {
    ...
    public async Task<(List<string> modified, List<string> newFiles, List<string> deleted, DateTime lastUpdated)> GetChangedFilesAsync()
    {
        var projectRoot = _projectRoot;
        var aiWorkspace = WorkspaceHelper.GetAiWorkspacePath(projectRoot);
        var indexFileName = WorkspaceHelper.GetIndexFileName(projectRoot);
        ...
        {
            ConsoleHelper.Warning($"Warning: Could not run git. Skipping new file detection. ({ex.Message})");
        }
    ...
    public async Task StatusAsync()
    {
        ...
        try
        {
            (modifiedFiles, newFiles, deletedFiles, lastUpdated) = await GetChangedFilesAsync();
        }
        catch (Exception ex)
        {
