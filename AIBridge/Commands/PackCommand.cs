...
/// <summary>
/// Handles packing source files into an AI context file.
/// </summary>
public class PackCommand
{
    private readonly string _projectRoot;

    public PackCommand(string projectRoot)
    {
    ...
    private static readonly string[] AlwaysExcludePrefixes =
    [
    ...
    /// </summary>
    private async Task<List<string>?> GetGitTrackedFiles(string projectPath)
    {
        try
        {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Git not available: {ex.Message}");
            return null;
        }
    }

    public async Task RunAsync(bool incremental = false)
    {
        ...
        if (incremental)
        {
            try
            {
                var indexCmd = new IndexCommand(_projectRoot);
                var (modified, newFiles, _, _) = await indexCmd.GetChangedFilesAsync();
        ...
        var gitFiles = await GetGitTrackedFiles(projectPath);
        string[] allFiles;
        ...
        foreach (var file in allFiles.OrderBy(f => f))
        {
            ...
            try
            {
                ...
                ConsoleHelper.Info($"  Packed: {relativePath}");
            }
            catch (Exception ex)
            {
